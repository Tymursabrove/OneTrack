<script lang="ts">
	...
	let icon: string = Icons.ga4;
	let title: string = 'Google Analytics 4';
	...
	async function handleSaveClick() {
		...
		if (!connector.data.measurementId) {
			showErrorMessage('Measurement Id value is required');
			return;
		}
		if (!connector.data.apiSecret) {
					...
					>
						...
						>
							<input
								...
								name="item_value"
								bind:value={connector.data.measurementId}
							/>
						</div>
					</div>
					...
					>
						...
						>
							<input
								...
								bind:value={connector.data.apiSecret}
							/>
						</div>
					</div>
