<script lang="ts">
	...
	export let growthRate: boolean;
	export let growth: string;
...
<div class="bg-bglinear-4 shadow-sidebar rounded-2xl border border-grey-4">
	<div class="2xl:px-5 p-3">
		...
		<div class="flex items-end gap-2">
			...
			<div class="flex gap-1 items-center">
				{#if growth}
					<Icon data={Icons.growthUp} stroke="transparent" class="w-3 text-green" />
				{:else}
					<Icon data={Icons.growthDown} stroke="transparent" class="w-3 text-red" />
				{/if}
	...
	<div class="h-10 rounded-b-2xl -mt-1 overflow-hidden">
		<Chart
			id="chart-card-1"
			type="area"
			height="100%"
