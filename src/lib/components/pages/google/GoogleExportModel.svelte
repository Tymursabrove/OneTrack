...
{#if isOpen}
	...
	>
		...
		<div
			class="relative transform max-h-full flex flex-col p-3 text-left transition-all sm:my-5 w-full max-w-sm rounded-3xl border border-grey-4 backdrop-blur-md before:bg-dropdown-2 before:opacity-30 before:shadow-dropdown-2 before:ease-in-out before:duration-300 before:absolute before:inset-0 before:rounded-3xl"
		>
			<div
			...
			>
				...
				<div class="flex gap-4 justify-center">
					...
					<button
						on:click={() => {
							CSVExportQue.set({
								googleCampaignsSelected: campaignsSelected,
								googleAdSetsSelected: adSetsSelected,
								googleAdsSelected: adsSelected
							});
							closeModal();
						}}
						type="button"
